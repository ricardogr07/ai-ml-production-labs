...
- Manual dispatch (`integration-test.yml`, lab `09-llamaindex-doc-qa-lab`, image name `llamaindex-doc-qa-lab`): builds and pushes the app image to GHCR, then `terraform apply` on `infra/main.tf` deploys three resources:
  ...
  - an API-key-protected Qdrant server on Azure Container Instances
...
The Ollama endpoint is public and unauthenticated for the minutes the run lasts, destroyed at the end of every run. Qdrant is reachable on a public IP but is API-key protected (a per-run key set on the server and presented by the app and the seed step), so a client without the key cannot read, seed, or delete the collection. See Production notes.

## Production notes

- The deployed Qdrant runs with `QDRANT__SERVICE__API_KEY` set to a per-run key (generated by CI, shared with the app and seed step, torn down with the run); a private-networking variant reachable only by the app and CI runner would harden it further. The local docker-compose Qdrant is loopback and stays keyless.
- The ACI images (`ollama/ollama`, `qdrant/qdrant`) are pinned to fixed tags and pulled with Docker Hub credentials, since anonymous `:latest` pulls from a shared ACI egress IP hit Docker Hub's rate limit and fail the apply.
