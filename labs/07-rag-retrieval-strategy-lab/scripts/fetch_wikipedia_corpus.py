#!/usr/bin/env python3
"""
One-off script: fetch real Wikipedia article summaries for the comparison corpus.
...
scripts/run_comparison.py. Not part of CI or any test path, keeps the
comparison offline and deterministic.

Usage:
  uv run python labs/07-rag-retrieval-strategy-lab/scripts/fetch_wikipedia_corpus.py
"""
...
_TOPICS = [
    ...
    ("Sourdough", "cooking"),
    ("Fermentation in food processing", "cooking"),
    ("Sous vide", "cooking"),
    ("Umami", "cooking"),
    ("Knife", "cooking"),
...
def _fetch_extract(title: str) -> str:
    ...
    req = urllib.request.Request(
        url, headers={"User-Agent": "ai-ml-production-labs/lab-07 (contact: repo owner)"}
    )
